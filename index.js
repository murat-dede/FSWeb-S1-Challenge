...
function indekseGoreFenomen(fenomenler,i) {
  const profil=fenomenler[i].profile
  return i+'. indekste bulunan fenomen: '+ profil
}
...
for (let i = 0; i < yenifenomenler; i++)

{
  const fenomen = yenifenomenler[i];
  yenifenomenler [i] = fenomen.profile;
}
...
function fenomenSil(gelendizi/indeks) {
 ...
 yenidizi.splice(indeks,1);
  ...
  console.log(fenomanSil(fenomenler,0) )
...
function fenomenEkle(Fenomeneler, gelenNumber,gelenProfile,gelenFollowers,gelenPosts,gelenPlatform) {
  let yeniObje= {
    number: gelenNumber,
...
console.log(fenomenEkle(fenomenler,1250,"Hakan",240,22"Instagram"));
...
function fenomenGonderimSayisi(arrayOfObject,fenomenName){
  ...
  if (fenomenName === arrayOfObject[i].profile) {
    fenomenPostSayısı = arrayOfObject[i].posts;
  }
  }
